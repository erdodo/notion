...
vi.mock('@/components/ui/command', () => ({
  ...
  CommandItem: ({ children, onSelect, value }: any) => (
    <div role="option" onClick={() => onSelect(value)} data-value={value}>
      {children}
    </div>
  ),
}));
...
describe('SearchCommand', () => {
  ...
  it('renders search input when open', async () => {
    ...
    expect(
      await screen.findByPlaceholderText('Search pages by title or content...')
    ).toBeInTheDocument();
  });
  ...
  it('creates new page on action click', async () => {
    ...
    await act(async () => {
      fireEvent.click(
        newPageAction.closest('[role="option"]') || newPageAction
      );
    });
