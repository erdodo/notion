    ...
    return (
        <div className="space-y-4 pt-2">
            ...
            <div className="space-y-1">
                <Reorder.Group axis="y" values={options} onReorder={onChange} className="space-y-1">
                    {options.map((option) => (
                                        ...
                                        <PopoverTrigger asChild>
                                            <div
                                                className={cn(
                                                    ...
                                                    NOTION_COLORS.find(c => c.value === option.color)?.bg || "bg-gray-100"
                                                )}
                                            >
                                                <div className={cn(
                                                    "w-1.5 h-1.5 rounded-full",
                                                    NOTION_COLORS.find(c => c.value === option.color)?.dot || "bg-gray-500"
                                                )} />
                                            </div>
                                        </PopoverTrigger>
                                            ...
                                            <div className="grid grid-cols-1 gap-0.5">
                                                {NOTION_COLORS.filter((v, i, a) => a.findIndex(t => (t.value === v.value)) === i).map(color => (
                                                    <div
                                                        ...
                                                        onClick={() => handleUpdateOption(option.id, { color: color.value })}
                                                    >
                                                        <div className={cn("w-4 h-4 rounded-sm flex items-center justify-center", color.bg)}>
                                                            <div className={cn("w-1.5 h-1.5 rounded-full", color.dot)} />
                                                        </div>
                                                        <span className="capitalize flex-1">{color.name}</span>
                                                        {option.color === color.value && <Check className="h-3 w-3" />}
                                                    </div>
                                                ))}
                                            </div>
                                        </PopoverContent>
            ...
            <div className="flex items-center gap-2 mt-2 pt-2 border-t">
                ...
                />
                <Button onClick={handleAddOption} size="sm" variant="secondary" className="h-8">
                    Add
                </Button>
            </div>
        </div>
