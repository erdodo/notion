...
export function RelationConfig({ config, currentDatabaseId, onChange }: RelationConfigProps) {
    const [databases, setDatabases] = useState<Database[]>([])
    const [loading, setLoading] = useState(true)

    const [targetDatabaseId, setTargetDatabaseId] = useState(config?.targetDatabaseId || '')
    const [bidirectional, setBidirectional] = useState(config?.bidirectional || false)
    const [limitType, setLimitType] = useState<'none' | 'one'>(config?.limitType || 'none')

    useEffect(() => {
        getAllDatabases()
            .then(dbs => setDatabases(dbs.filter(d => d.id !== currentDatabaseId)))
    ...
    return (
        <div className="space-y-4 p-4">
                    ...
                    <SelectContent>
                        {databases.map(db => (
                            <SelectItem key={db.id} value={db.id}>
                                {/* @ts-ignore */}
                                {db.page?.icon} {(db as any).page?.title}
                            </SelectItem>
                        ))}
                    </SelectContent>
                </Select>
            ...
            <div className="space-y-2">
                <Label>Limit</Label>
                <Select value={limitType} onValueChange={(v) => setLimitType(v as 'none' | 'one')}>
                    <SelectTrigger>
                        <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
            ...
            <Button onClick={handleSave} className="w-full" disabled={!targetDatabaseId}>
                Done
            </Button>
        </div>
    )
