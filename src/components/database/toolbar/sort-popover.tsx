...
interface SortPopoverProperties {
  properties: Property[];
}

export function SortPopover({ properties }: SortPopoverProperties) {
        ...
        <div className="space-y-3">
          ...
          {sorts.map((sort, index) => {
            ...
            return (
              <div
                key={index}
                className="flex items-center gap-2 text-sm bg-secondary/50 p-2 rounded border border-border/50"
              >
                <span className="text-muted-foreground font-medium flex-1 truncate">
                  {property.name}
                </span>
                <span
                  className="text-xs text-muted-foreground flex items-center gap-1 cursor-pointer hover:text-foreground transition-colors select-none"
                  onClick={() => {
                    updateSort(index, {
                      ...sort,
