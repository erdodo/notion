...
import { Filter, X, Plus, Calendar as CalendarIcon, Check } from "lucide-react"
...
const getOperatorsForType = (type: PropertyType): { label: string, value: FilterOperator }[] => {
    switch (type) {
        case 'TEXT':
        case 'URL':
        case 'EMAIL':
            ...
            <PopoverContent className="w-[400px] p-3" align="start">
                <div className="space-y-3">
                    ...
                    {filters.map((filter) => {
                        ...
                        if (!property) return null // Should remove invalid filter?

                        const operators = getOperatorsForType(property.type)

                        return (
                            <div key={index} className="flex flex-col gap-2 p-2 rounded bg-secondary/30 border border-border/50">
                                <div className="flex items-center gap-2 text-sm">
                                    <span className="text-muted-foreground min-w-[80px] truncate font-medium" title={property.name}>{property.name}</span>
                                    <Select
                                        value={filter.operator}
                                        onValueChange={(val) => updateFilter(index, { ...filter, operator: val as FilterOperator })}
                                    >
                                    ...
                                    <Button variant="ghost" size="icon" className="h-5 w-5 ml-auto text-muted-foreground hover:text-destructive" onClick={() => removeFilter(index)}>
                                        <X className="h-3 w-3" />
                                    </Button>
                                </div>

                                {!['is_empty', 'is_not_empty', 'is_checked', 'is_unchecked', 'is_today', 'is_tomorrow', 'is_yesterday', 'is_one_week_ago', 'is_one_month_ago'].includes(filter.operator) && (
                                    <div className="pl-[88px]">
                                        {(property.type === 'DATE' || property.type === 'CREATED_TIME' || property.type === 'UPDATED_TIME') ? (
                                            <Popover>
                                                <PopoverTrigger asChild>
                                                    <Button variant={"outline"} className={cn("w-full pl-3 text-left font-normal h-8 text-xs", !filter.value && "text-muted-foreground")}>
                                                ...
                                                <PopoverContent className="w-auto p-0" align="start">
                                                    <Calendar
                                                        ...
                                                        onSelect={(date) => updateFilter(index, { ...filter, value: date?.toISOString() })}
                                                        initialFocus
                                                    />
                                                </PopoverContent>
                                            </Popover>
                                            ...
                                            <CommandItem
                                                ...
                                                onSelect={() => {
                                                    addFilter({
                                                        ...
                                                        operator: getOperatorsForType(property.type)[0].value,
                                                        value: ''
                                                    } as any)
                                                    setAddOpen(false)
                                                }}
