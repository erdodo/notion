...
import { MessageSquare, Send, Reply, Check, Trash2 } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    socket.on('comment-deleted', (data: { commentId: string }) => {
      setComments((previous) =>
        previous.filter((c) => c.id !== data.commentId)
      );
    });
...
function CommentItem({
  ...
  return (
    <div className={cn('group', comment.resolved && 'opacity-60')}>
      <div className="flex gap-3">
