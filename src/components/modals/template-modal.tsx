...
export const TemplateModal = () => {
    ...
    const onSelect = async (template: any) => {
        ...
        try {
            await applyTemplate(params.documentId as string, template.id)
            // Force a full page reload to ensure all stores (websocket, collaboration, editor) are reset.
            // This is necessary because server actions and soft refreshes might not trigger
            // deeply nested store re-initializations immediately.
                        ...
                        {templates.map((template) => (
                            ...
                            >
                                ...
                                <div className="mt-auto p-4 pt-0">
                                    ...
                                    >
                                        {isLoading ? <Loader2 className="h-3 w-3 animate-spin" /> : "Use template"}
                                    </button>
                                </div>
                            </div>
                        ))}
