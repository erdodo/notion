...
vi.mock('react-dropzone', () => ({
  useDropzone: vi.fn(({ onDrop }) => ({
    getRootProps: () => ({
      onClick: () => {
        const files = [new File(['content'], 'test.md', { type: 'text/markdown' })]
        onDrop(files)
      },
      onDrop: (e: any) => {
        const files = [new File(['content'], 'test.md', { type: 'text/markdown' })]
...
describe('ImportModal', () => {
  ...
  it('should switch between import types', async () => {
    ...
    expect(csvTab).toHaveClass('bg-secondary') // Variant changes to secondary when selected
  })
  ...
  it('should show loading state during import', async () => {
    ...
    // Simulate file drop - this is already mocked in useDropzone
  ...
  it('should show result message on successful import', async () => {
    global.fetch = vi.fn(() =>
      Promise.resolve({
        ok: true,
        json: () => Promise.resolve({ success: true, message: 'Import successful' }),
      })
    ) as any
  ...
  it('should handle different import formats', async () => {
    ...
    // Test Markdown
    let markdownTab = screen.getByRole('button', { name: /markdown/i })
    expect(markdownTab).toHaveClass('bg-secondary')
