...
describe('CoverImageModal', () => {
  ...
  it('should handle image file upload', async () => {
    const mockEdgeStore = { publicFiles: { upload: vi.fn().mockResolvedValue({ url: 'https://example.com/uploaded.jpg' }) } }
  ...
  it('should load recent images from localStorage on mount', () => {
    const recentImages = ['https://example.com/1.jpg', 'https://example.com/2.jpg']
  ...
  it('should call onConfirm with selected URL', async () => {
    ...
    expect(mockOnConfirm).toHaveBeenCalledWith(expect.stringContaining('https://example.com/image.jpg'))
  })

  it('should show loading state during upload', async () => {
    const mockEdgeStore = {
      publicFiles: {
        upload: vi.fn(() => new Promise(() => {})), // Never resolves
      },
    }
    ;(useEdgeStore as any).mockReturnValue({ edgestore: mockEdgeStore })
    ...
    const uploadImageButton = screen.getByRole('button', { name: /upload image/i })
  ...
  it('should handle image URL validation', async () => {
    ...
    // Component should validate the URL
    expect(input.value).toBe('invalid-url')
  })

  it('should limit recent images to 9', () => {
    const manyImages = Array.from({ length: 15 }, (_, i) => `https://example.com/${i}.jpg`)
  ...
  it('should display preview of selected image', async () => {
    ...
    // Component should show image preview
