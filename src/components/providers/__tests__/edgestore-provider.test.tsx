...
vi.mock('@/lib/edgestore', () => ({
  EdgeStoreProvider: ({ children }: any) => (
    <div data-testid="edgestore-provider">{children}</div>
  ),
}));
...
describe('EdgeStoreProviderWrapper', () => {
  ...
  it('should handle empty children', () => {
    const { container } = render(
      <EdgeStoreProviderWrapper>{null}</EdgeStoreProviderWrapper>
    );

    const edgestoreProvider = container.querySelector(
      '[data-testid="edgestore-provider"]'
    );
