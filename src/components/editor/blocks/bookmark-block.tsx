...
export const BookmarkBlock = createReactBlockSpec(
    ...
    },
    {
        render: (props) => {
            ...
            const [metadata, setMetadata] = useState<{ title?: string; description?: string; image?: { url: string }; icon?: string } | null>(null)
            ...
            if (!url) {
                return (
                    <div className="p-3 bg-muted/20 border rounded flex gap-2 items-center my-2" contentEditable={false}>
                            ...
                            <div className="flex flex-col md:flex-row h-full">
                                <div className="flex-1 p-3 overflow-hidden flex flex-col justify-between">
                                    ...
                                    <div className="flex items-center gap-2 mt-auto">
                                        ...
                                        {metadata?.icon && <img src={metadata.icon} alt="" className="w-4 h-4 object-contain" />}
                                ...
                                {metadata?.image?.url && (
                                    <div className="w-full md:w-1/3 h-32 md:h-28 overflow-hidden bg-muted relative border-l">
                                        {/* eslint-disable-next-line @next/next/no-img-element */}
                                        <img src={metadata.image.url} alt="Preview" className="w-full h-full object-cover" />
                                    </div>
                                )}
                            </div>
                        )}
