...
import { Video, Loader2, Link as LinkIcon, UploadCloud, Film } from "lucide-react"
import { cn } from "@/lib/utils"
import { getEmbedUrl, isVideoUrl } from "@/lib/embed-utils" // We will create this or use inline if not
    ...
    },
    {
        render: (props) => {
            ...
            const handleUrlSubmit = () => {
                if (!inputUrl) return
                // Ideally we validate youtube/vimeo here, but allow any url as generic video source if not matched too
                try {
                    new URL(inputUrl)
                    editor.updateBlock(block, {
                        props: { url: inputUrl }
            ...
            if (block.props.url) {
                ...
                // If it is an embed (Youtube/Vimeo), use iframe
                if (isEmbed) {
                    return (
                ...
                // Otherwise assume direct video file (mp4, etc)
                return (
                        ...
                        {activeTab === "upload" ? (
                            <>
                                <div className="bg-background rounded-full p-4 shadow-sm border">
                                    {uploading ? <Loader2 className="animate-spin text-muted-foreground" /> : <Film className="text-muted-foreground" />}
                                </div>
