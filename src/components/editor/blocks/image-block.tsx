...
import { ImageIcon, Loader2, Link as LinkIcon, UploadCloud, AlignLeft, AlignCenter, AlignRight, Maximize } from "lucide-react"
        ...
        render: (props) => {
            ...
            useEffect(() => {
                const onMouseMove = (e: MouseEvent) => {
                    ...
                    // Adjust width based on alignment? Simplified for now: just resizing width
                    // For center align, we might want to scale symmetrically, but let's keep it simple
                    const newWidth = Math.max(100, Math.min(1200, startWidth + diff * (block.props.align === "center" ? 2 : 1)))
            ...
            if (block.props.url) {
                return (
                    <div className={cn("group flex flex-col my-4 relative",
                        ...
                        <div
                            ...
                            style={{
                                width: block.props.align === "full" ? "100%" : `${block.props.width}px`,
                                maxWidth: "100%"
                            }}
                        >
                            {/* Media Content */}
                            ...
                            <div className="absolute top-2 right-2 flex gap-1 bg-black/50 backdrop-blur-sm rounded p-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button
                                    onClick={() => editor.updateBlock(block, { props: { align: "left" } })}
                                    className={cn("p-1 rounded hover:bg-white/20 text-white", block.props.align === "left" && "bg-white/30")}
                                >
                                    <AlignLeft size={14} />
                                </button>
                                <button
                                    onClick={() => editor.updateBlock(block, { props: { align: "center" } })}
                                    className={cn("p-1 rounded hover:bg-white/20 text-white", block.props.align === "center" && "bg-white/30")}
                                >
                                    <AlignCenter size={14} />
                                </button>
                                <button
                                    onClick={() => editor.updateBlock(block, { props: { align: "right" } })}
                                    className={cn("p-1 rounded hover:bg-white/20 text-white", block.props.align === "right" && "bg-white/30")}
                                >
                                    <AlignRight size={14} />
                                </button>
                                <button
                                    onClick={() => editor.updateBlock(block, { props: { align: "full" } })}
                                    className={cn("p-1 rounded hover:bg-white/20 text-white", block.props.align === "full" && "bg-white/30")}
                                >
                                    <Maximize size={14} />
                                </button>
                            </div>
                    ...
                    <div className="rounded-md border border-dashed p-8 bg-muted/20 hover:bg-muted/40 transition-colors flex flex-col items-center justify-center gap-4">
                        <div className="flex gap-4 mb-4 border-b w-full justify-center pb-2">
                            <button
                                ...
                                className={cn("text-sm font-medium transition-colors pb-2 -mb-2.5 border-b-2", activeTab === "upload" ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground")}
                            >
                                Upload
                            </button>
                            <button
                                onClick={() => setActiveTab("embed")}
                                className={cn("text-sm font-medium transition-colors pb-2 -mb-2.5 border-b-2", activeTab === "embed" ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground")}
                            >
                                Embed Link
                            </button>
                        </div>
                        ...
                        {activeTab === "upload" ? (
                            <>
                                <div className="bg-background rounded-full p-4 shadow-sm border">
                                    {uploading ? <Loader2 className="animate-spin text-muted-foreground" /> : <UploadCloud className="text-muted-foreground" />}
                                </div>
