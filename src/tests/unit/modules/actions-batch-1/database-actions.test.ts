...
vi.mock('@/lib/db', () => ({
    db: {
        ...
        $transaction: vi.fn((cb) => cb && typeof cb === 'function' ? cb(db) : cb)
    }
}))
...
describe('Database Actions', () => {
    ...
    describe('createDatabase', () => {
        it('creates page and database successfully', async () => {
            ...
            const mockDb = { id: 'db-1', pageId: 'page-1' }

            vi.mocked(db.page.create).mockResolvedValue(mockPage as any)
            vi.mocked(db.database.create).mockResolvedValue(mockDb as any)
            // Mock addRow call inside createDatabase (needs recursive mock logic or just let calls happen)
            vi.mocked(db.databaseRow.count).mockResolvedValue(0)
            vi.mocked(db.databaseRow.create).mockResolvedValue({ id: 'row-1' } as any)

            // Mock db.database.findUnique used in addRow
            vi.mocked(db.database.findUnique).mockResolvedValue(mockDb as any)
            ...
            // Should create a default row
            expect(db.databaseRow.create).toHaveBeenCalled()
            expect(result).toEqual({ page: mockPage, database: mockDb })
        })
    ...
    describe('getDatabase', () => {
        it('returns database with relations', async () => {
            const mockDb = { id: 'db-1', pageId: 'p-1' }
            vi.mocked(db.database.findUnique).mockResolvedValue(mockDb as any)

            const result = await dbActions.getDatabase('p-1')
            expect(db.database.findUnique).toHaveBeenCalledWith(expect.objectContaining({
                where: { pageId: 'p-1' },
    ...
    describe('addRow', () => {
        it('creates a row and page', async () => {
            const dbId = 'db-1'
            const mockDb = { id: dbId, pageId: 'p-main' }
            vi.mocked(db.database.findUnique).mockResolvedValue(mockDb as any)
            vi.mocked(db.page.create).mockResolvedValue({ id: 'p-row' } as any)
            vi.mocked(db.databaseRow.create).mockResolvedValue({ id: 'row-1' } as any)

            await dbActions.addRow(dbId)

            expect(db.page.create).toHaveBeenCalledWith(expect.objectContaining({
                data: expect.objectContaining({ parentId: 'p-main' })
            }))
            expect(db.databaseRow.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('deleteRow', () => {
        it('deletes the page associated with the row', async () => {
            const rowId = 'row-1'
            vi.mocked(db.databaseRow.findUnique).mockResolvedValue({ id: rowId, pageId: 'p-row' } as any)
        ...
        it('deletes just row if no page (fallback)', async () => {
            const rowId = 'row-1'
            vi.mocked(db.databaseRow.findUnique).mockResolvedValue({ id: rowId, pageId: null } as any)

            await dbActions.deleteRow(rowId)

            expect(db.databaseRow.delete).toHaveBeenCalledWith({ where: { id: rowId } })
        })
    })
    ...
    describe('updateCell', () => {
        it('updates cell value and triggers automation', async () => {
            ...
            vi.mocked(db.cell.update).mockResolvedValue({ id: cellId } as any)
            ...
            expect(db.cell.update).toHaveBeenCalledWith({ where: { id: cellId }, data: { value: newValue } })
        })
    })
    ...
    describe('addProperty', () => {
        it('creates property', async () => {
            ...
            vi.mocked(db.property.create).mockResolvedValue({ id: 'prop-1' } as any)
