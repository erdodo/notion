...
describe('Block Actions', () => {
    ...
    describe('updateSyncedBlockContent', () => {
        ...
        it('updates synced block content correctly', async () => {
            ...
            const result = await BlockActions.updateSyncedBlockContent('page-1', 'synced-1', newChildren)
            ...
            // Check if content was actually updated in the JSON string passed to db.update
            const updateCallArgs = vi.mocked(db.page.update).mock.calls[0][0]
            const updatedContent = JSON.parse(updateCallArgs.data.content as string)
        ...
        it('updates regular block children correctly', async () => {
            ...
            const result = await BlockActions.updateSyncedBlockContent('page-1', 'block-1', newChildren)
        ...
        it('updates nested block correctly', async () => {
            ...
            await BlockActions.updateSyncedBlockContent('page-1', 'block-2', newChildren)

            const updateCallArgs = vi.mocked(db.page.update).mock.calls[0][0]
            const updatedContent = JSON.parse(updateCallArgs.data.content as string)
            // Navigate to nested block to check
        ...
        it('returns error if block not found', async () => {
            const result = await BlockActions.updateSyncedBlockContent('page-1', 'non-existent', [])
        ...
        it('throws unauthorized if user not logged in', async () => {
            vi.mocked(auth).mockResolvedValue(null)
            await expect(BlockActions.updateSyncedBlockContent('page-1', 'block-1', []))
