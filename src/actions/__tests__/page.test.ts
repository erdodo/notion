...
describe('Page Actions', () => {
  ...
  describe('getPageById', () => {
    ...
    it('throws unauthorized if user not logged in', async () => {
      vi.mocked(auth).mockResolvedValue(null);
      await expect(PageActions.getPageById('page-1')).rejects.toThrow(
        'Unauthorized'
      );
    });
  });
  ...
  describe('archivePage', () => {
    it('archives page and children recursively', async () => {
      ...
      vi.mocked(db.page.findMany).mockResolvedValueOnce(mockChildren as any);
  ...
  describe('restorePage', () => {
    ...
    it('throws if page not found', async () => {
      ...
      await expect(PageActions.restorePage('missing-id')).rejects.toThrow(
        'Page not found'
      );
    });
  });
