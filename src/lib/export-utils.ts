...
function tableToMarkdown(block: any): string {
    // Table block'u markdown table'a çevir
    ...
    md += "| " + headerCells.map((c: any) => contentToMarkdown(c)).join(" | ") + " |\n"
    md += "| " + headerCells.map(() => "---").join(" | ") + " |\n"

    // Data rows
    for (let i = 1; i < rows.length; i++) {
        const cells = rows[i]?.cells || []
        md += "| " + cells.map((c: any) => contentToMarkdown(c)).join(" | ") + " |\n"
    }
...
function blockToHTML(block: any): string {
    switch (block.type) {
        ...
        case "image":
            const imgCaption = block.props?.caption ? `<figcaption>${escapeHtml(block.props.caption)}</figcaption>` : ""
...
export function formatCellValueForCSV(
    ...
    // value objesi içinde value alanı olabilir
    ...
    switch (propertyType) {
        ...
        case "SELECT":
            // Select value bir option ID olabilir, direkt string de olabilir
            return String(actualValue || "")
